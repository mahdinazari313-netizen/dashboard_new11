...
/**
 * هر ردیف = یک Symbol که حداقل یک سیگنال قابل‌نمایش دارد (بخش ۱۸ سند).
 * داخل هر کارت، تمام سیگنال‌های فعال آن Symbol (هر تایم‌فریم/جهت) لیست می‌شوند.
 *
 * این Adapter هیچ فیلتر Scenario/Price-Difference/Main-Time-Frame ندارد
 * (بخش ۱۸ صریحاً: «صفحه اول شروط سناریو را اعمال نمی‌کند»).
 * تنها فیلتری که اینجا اعمال می‌شود Display Duration است که در Fragment محاسبه می‌شود.
 */
